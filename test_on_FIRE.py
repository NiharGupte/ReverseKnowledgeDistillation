...
print(config['PREDICT']['model_save_path'])
...
for pair_file in tqdm(match_pairs):
    ...
    if inliers_num_rate < 1e-6:
        print("Failed : ", query_im_path, refer_im_path)
        f.write("Failed : " + query_im_path + "\n")
        failed += 1
        avg_dist = big_num
    else:
        ...
        final_mee.append(mee)
        
        if mae > 50 or mee > 20:
            print("Inaccurate : ", query_im_path, refer_im_path)
            #f.write("Inaccurate : " + query_im_path + "\n")
            print("MAE : ", mae, " MEE : ", mee)
            inaccurate += 1
    
    auc_record[category].append(avg_dist)

# np.savetxt("KD_MAE.txt", np.array(final_mae))
# np.savetxt("KD_MEE.txt", np.array(final_mee))
...
# log_dict = {
#                 'Failed': (100*failed/image_num), 
#                 'Acceptable': (100*(image_num-inaccurate-failed)/image_num),
#                 'Inaccurate':(100*inaccurate/image_num),
#                 'mAUC': auc['mAUC'],
#                 'AUC[S]': auc['s'],
#                 'AUC[P]': auc['p'],
#                 'AUC[A]': auc['a']
