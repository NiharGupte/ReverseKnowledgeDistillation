...
def train_model(model, optimizer, dataloaders, device, num_epochs, train_config):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in phases:
            ...
            print_knowledge_detect = []
            print_knowledge_descript = []
            ...
            for images, input_with_label, keypoint_positions, label_names \
                ...
                with torch.set_grad_enabled('train' in phase):
                    ...
                    print_knowledge_detect.append(model.return_knowledge_losses()[0])
                    print_knowledge_descript.append(model.return_knowledge_losses()[1])
            ...
            print_knowledge_detect = sum(print_knowledge_detect)/len(print_knowledge_detect)
            print_knowledge_descript = sum(print_knowledge_descript)/len(print_knowledge_descript) 
            
            print(phase, 'overall loss: {}'.format(print_detector_loss + print_descriptor_loss + print_knowledge_detect + print_knowledge_descript),
            ...
            if 'train' in phase:
                log_dict = {
                    'train overall loss': print_detector_loss + print_descriptor_loss, 
                    'train detector loss': print_detector_loss,
                    'train avg keypoints learned':num_learned_pts / num_input_with_label,
                    'train descriptor loss': print_descriptor_loss,
                    'global epoch':epoch,
                    'Knowledge_Loss_descript' : print_knowledge_descript,
                    'Knowledge_Loss_detect' : print_knowledge_detect
                }
            else:
                log_dict = {
                    'val overall loss': print_detector_loss + print_descriptor_loss, 
                    'val detector loss': print_detector_loss,
                    'val avg keypoints learned':num_learned_pts / num_input_with_label,
                    'val descriptor loss': print_descriptor_loss,
                    'val Knowledge_Loss_descript' : print_knowledge_descript,
                    'val Knowledge_Loss_detect' : print_knowledge_detect 
                }
            wandb.log(log_dict)
            ...
            for s, (name, _) in enumerate(show_names):
                ...
                plt.show()
                # plt.savefig(f'./data/train_images/pretrained_image_with_keypoints_{epoch}.png')
                wandb.log({"image_with_keypoints" : plt})
                plt.figure(dpi=200)
                plt.subplot(121)
