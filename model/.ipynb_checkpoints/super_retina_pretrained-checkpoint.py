...
from model.super_retina import SuperRetinaTeacher
from common.common_util import remove_borders, sample_keypoint_desc, simple_nms, nms, \
    sample_descriptors,sample_descriptors_knowledge
...
class SuperRetina(nn.Module):
    ...
    def __init__(
        ...
        device='cuda',
        n_class = 1,
        img_size: Sequence[int] | int = (768,768),
        in_channels: int = 1,
        out_channels: int = 1,
        ...
        self.convDa = torch.nn.Conv2d(96, 256, kernel_size=3, stride=1, padding=1)
        self.convDb = torch.nn.Conv2d(256, 256, kernel_size=4, stride=2, padding=0)
        self.convDc = torch.nn.Conv2d(256, 256, kernel_size=1, stride=1, padding=0)

        self.trans_conv = nn.ConvTranspose2d(256, 256, 2, stride=2)

        self.out = UnetOutBlock(spatial_dims=spatial_dims, in_channels=feature_size, out_channels=out_channels)
        
        checkpoint = torch.load("./save/SuperRetina.pth", map_location=device)
        self.teacher = SuperRetinaTeacher()
        self.teacher.load_state_dict(checkpoint['net'])
        for param in self.teacher.parameters():
            param.requires_grad = False
        
        self.teacher_detector_loss = DiceLoss()
        self.l1 = 0
        self.l2 = 0
    ...
    def load_from(self, weights):
        with torch.no_grad():
            print("Load weights : ", self.swinViT.patch_embed.proj.weight.shape)
...
#         print("This one is called now")
        hidden_states_out = self.swinViT(x_in, self.normalize) 
  #     [List of 5 states]         x = (1, 768, 768)
        enc0 = self.encoder1(x_in) 
  # (1,24,768,768)       (1,24,768,768)
      
        enc1 = self.encoder2(hidden_states_out[0]) #(24, 384, 384)
  # (1,24,384,384)              (1,24,384,384)
        
        enc2 = self.encoder3(hidden_states_out[1]) #(48, 192, 192)
  # (1,48,192,192)              (1,48,192,192)      
       
        enc3 = self.encoder4(hidden_states_out[2]) #(96, 96, 96)
  # (1,96,96,96)              (1,96,96,96)      
        
        dec4 = self.encoder10(hidden_states_out[4]) #(384, 24, 24)
  # (1,384,24,24)             (1,384,24,24)      
        
        dec3 = self.decoder5(dec4, hidden_states_out[3]) #(192, 48, 48)
  # (1,192,48,48)       (1,384,24,24) (1,192,48,48)      
        
        dec2 = self.decoder4(dec3, enc3)
  # (1,96,96,96)       (1,192,48,48) (1,96,96,96)      
        
############### THIS IS WHERE DESCRIPTOR WILL GO
        #96->256, 256->256, 256->256 
        cDa = self.relu(self.convDa(dec2))
...
################ DESCRIPTOR ENDS HERE

        dec1 = self.decoder3(dec2, enc2)
  # (1,48,192,192)     (1,96,96,96) (1,48,192,192)      

        dec0 = self.decoder2(dec1, enc1) 
  # (1,24,384,384)     (1,48,192,192) (1,24,384,384)     
        
        out = self.decoder1(dec0, enc0)
  # (1,24,768,768)     (1,24,384,384) (1,24,768,768)     
        logits = self.out(out)
        semi = torch.sigmoid(logits)
        return semi, desc
    
    def knowledge_forward(self, xin):
        with torch.no_grad():
            s_teacher, d_teacher = self.teacher.forward1(xin)
        return s_teacher, d_teacher
    
    def descriptor_knowledge_loss(self, detector_pred, label_point_positions, descriptor_pred,
                        affine_descriptor_pred, affine_detector_pred=None):
        ...
        """
#         print("Knowledge descriptor loss func")
...
#         print("Label : ", label_point_positions)
        detector_pred[label_point_positions == 1] = 10
        
#         print("Detector Pred " , detector_pred.shape)
#         print("Descriptor Pred " , (descriptor_pred).shape)
#         print("Affine descriptor Pred " , (affine_descriptor_pred).shape)
#         print("Detector Pred " , (detector_pred).shape)
        
        
        
        descriptors, affine_descriptors, keypoints = \
            sample_descriptors_knowledge(detector_pred, descriptor_pred, affine_descriptor_pred,
                               nms_size=self.nms_size, nms_thresh=self.nms_thresh, scale=self.scale,
                               affine_detector_pred=affine_detector_pred)
#         print(descriptors[0].shape)
#         print(affine_descriptors[0].shape)
        ...
        for i in range(len(affine_descriptors)):
            ...
            if n > 1000:  # avoid OOM
#                 print("Returns from n knowledge : ", n)
        ...
        if len(positive) == 0:
#             print("Returns from positive knowledge")
            return torch.tensor(0., requires_grad=True).to(descriptor_pred), False
    ...
    def descriptor_loss(self, detector_pred, label_point_positions, descriptor_pred,
        ...
        for i in range(len(affine_descriptors)):
            ...
            n = affine_descriptors[i].shape[1]
            if n > 1000:  # avoid OOM
#                 print("Returns from n normal affine : ", n)
                return torch.tensor(0., requires_grad=True).to(descriptor_pred), False
        ...
        if len(positive) == 0:
#             print("Returns from positive normal affine")
            return torch.tensor(0., requires_grad=True).to(descriptor_pred), False
    ...
    def forward(self, x, label_point_positions=None, value_map=None, learn_index=None):
        ...
        s_teacher, d_teacher = self.knowledge_forward(x)
        
        self.l1 = self.teacher_detector_loss(s_teacher, detector_pred)
        ...
        if label_point_positions is not None:
            ...
            self.l2, _  = self.descriptor_knowledge_loss(s_teacher.clone().detach(), label_point_positions, d_teacher, descriptor_pred)
            
            if self.PKE_learn and len(learn_index[0]) != 0:
                value_map[learn_index] = value_map_update
                
            loss = loss_detector + loss_descriptor + self.l1 + self.l2
    ...
    def return_knowledge_losses(self):
        return self.l1.cpu().data.sum(), self.l2.cpu().data.sum()
